...
public class BattleScript : MonoBehaviour
{
    ...
    void BattleSequence()
    {
        ...
        switch (currentState)
        {
                ...
                if (!damageProcessing)
                {
                    ...
                    else if (randomAttack == 1)
                    {
                        ...
                        if (enemySpellType[randomSpell] == "Single")
                        {
                            ...
                            StartCoroutine(damageDisplay(currentEnemyAttackString));
                        }
            ...
            case (BattleState.SELECTIONSINGLE):
                ...
                if (initialChoice)
                {
                    if (isRevive)
                    {
                        ...
                        {
                            if (selectionArray[player].GetComponent<BaseEntityClass>().getIsAlive() == false && selectionArray[player].GetComponent<BaseEntityClass>().getIsPlayer() == true)
                            {
                    ...
                    {
                        for (int enemy = 0; enemy < selectionArray.Length - 1; enemy++)
                        {
                            if (selectionArray[enemy].GetComponent<BaseEntityClass>().getIsAlive() == true && selectionArray[enemy].GetComponent<BaseEntityClass>().getIsPlayer() == false)
                            {
                                currentSelection = selectionArray[enemy];
                                currSel = enemy;

                                break;
                            }
                            currentSelection = selectionArray[4];
                        }
                    }
                    else
                    {
                        for (int player = 0; player < selectionArray.Length - 1; player++)
                        {
                            if (selectionArray[player].GetComponent<BaseEntityClass>().getIsAlive() == true && selectionArray[player].GetComponent<BaseEntityClass>().getIsPlayer() == true)
                            {
                                currentSelection = selectionArray[player];
                                currSel = player;
            ...
            case (BattleState.EXECUTIONOFATTACK):
                ...
                System.Reflection.MethodInfo theMethod = thisType.GetMethod(currentAttackString);
                    ...
                    if (false == isMulti)
                    {
                        ...
                        StartCoroutine(damageDisplay(currentAttackString));
                    }
                    else
                    {
            ...
            case (BattleState.PROCESSING):
                ...
                damageIndicator2.GetComponent<Text>().text = "";
                damageIndicator3.GetComponent<Text>().text = "";
                damageIndicator4.GetComponent<Text>().text = "";
                runText.text = "";
                
                damageIndicator.SetActive(false);
                damageIndicator2.SetActive(false);
                damageIndicator3.SetActive(false);
                damageIndicator4.SetActive(false);
                ...
                if (true == turnArray[currentTurn].GetComponent<BaseEntityClass>().getIsPlayer())
                {
                    if (true == turnArray[currentTurn].GetComponent<BaseEntityClass>().getIsAlive() && false == turnArray[currentTurn].GetComponent<BaseEntityClass>().getEscapedBattle())
                    {
                        ...
                        attackOne.GetComponent<Button>().onClick.AddListener(delegate { changeStateToSelection("Attack", false, true); });
            ...
            for (int buttonCount = 2; buttonCount < numButtons + 1; buttonCount++)
            {
                ...
                if (!tempAbility.Equals(""))
                {
                    if (targetType.Equals("Single"))
                    {
                        if (buttonCount == 2)
                        {
                            Attack2.onClick.AddListener(delegate { changeStateToSelection(tempAbility, reviveSkill, attackType); });
                        }
                        if (buttonCount == 3)
                        {
                            Attack3.onClick.AddListener(delegate { changeStateToSelection(tempAbility, reviveSkill, attackType); });
                        }
                        if (buttonCount == 4)
                        {
                            Attack4.onClick.AddListener(delegate { changeStateToSelection(tempAbility, reviveSkill, attackType); });
                        }
                    }
            ...
            for (int buttonCount = 1; buttonCount < numButtons + 1; buttonCount++)
            {
                ...
                if (!tempSpell.Equals(""))
                {
                    if (targetType.Equals("Single"))
                    {
                        if (buttonCount == 1)
                        {
                            Spell1.onClick.AddListener(delegate { changeStateToSelection(tempSpell, reviveSkill, attackType); });
                        }
                        if (buttonCount == 2)
                        {
                            Spell2.onClick.AddListener(delegate { changeStateToSelection(tempSpell, reviveSkill, attackType); });
                        }
                        if (buttonCount == 3)
                        {
                            Spell3.onClick.AddListener(delegate { changeStateToSelection(tempSpell, reviveSkill, attackType); });
                        }
                        if (buttonCount == 4)
                        {
                            Spell4.onClick.AddListener(delegate { changeStateToSelection(tempSpell, reviveSkill, attackType); });
                        }
                    }
                    else if (targetType.Equals("Multi"))
                    {
                        if (buttonCount == 1)
                        {
                            Spell1.onClick.AddListener(delegate { changeStateToSelectionMulti(tempSpell, attackType); });
                        }
                        if (buttonCount == 2)
                        {
                            Spell2.onClick.AddListener(delegate { changeStateToSelectionMulti(tempSpell, attackType); });
                        }
                        if (buttonCount == 3)
                        {
                            Spell3.onClick.AddListener(delegate { changeStateToSelectionMulti(tempSpell, attackType); });
                        }
                        if (buttonCount == 4)
                        {
                            Spell4.onClick.AddListener(delegate { changeStateToSelectionMulti(tempSpell, attackType); });
                        }
                    }
                    else if (targetType.Equals("Self"))
                    {
    ...
    public void changeStateToSelection(string thisAttack, bool isReviveSkill, bool isAttackSkill)
    {
    ...
    public void changeStateToSelectionMulti(string thisAttack, bool isAttackSkill)
    {
    ...
    void lairAction()
    {
        if (lairCounter >= 10)
        {
            foreach (GameObject playerNum in playerArray)
            {
                ...
                StartCoroutine(damageDisplay("LairAction"));
            }
            lairCounter = 0;
        }
        else
    ...
    IEnumerator damageDisplay(string stringAttack)
    {
        ...
        //Single target
        string damageVal = turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamageValue().ToString();
        string manaRegen = turnArray[currentTurn].GetComponent<BaseEntityClass>().getManaRegen().ToString();
        string healthRegen = turnArray[currentTurn].GetComponent<BaseEntityClass>().getHealValue().ToString();

        //Multi attack
        string damageVal1 = turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage1().ToString();
        string damageVal2 = turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage2().ToString();
        string damageVal3 = turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage3().ToString();
        string damageVal4 = turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage4().ToString();

        //Multi Heal
        string healVal1 = turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal1().ToString();
        string healVal2 = turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal2().ToString();
        string healVal3 = turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal3().ToString();
        string healVal4 = turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal4().ToString();

        //Scorpion boss lair action
        if (stringAttack == "LairAction")
        {
            runText.text = turnArray[currentTurn].GetComponent<BaseEntityClass>().getJob() + "caused stalactites to fall from the ceiling, inflicting ";

            if (target1.getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage1() > 0)
            {
                runText.text += damageVal1 + " damage to " + target1.getJob() + "! ";
            }

            if(target2.getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage2() > 0)
            {
                runText.text += damageVal2 + " damage to " + target2.getJob() + "! ";
            }

            if (target3.getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage3() > 0)
            {
                runText.text += damageVal3 + " damage to " + target3.getJob() + "! ";
            }

            if (target4.getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage4() > 0)
            {
                runText.text += damageVal4 + " damage to " + target4.getJob() + "! ";
            }
        ...
        //Self-abilities
        else if (stringAttack == "Sentinel")
        {
            runText.text = turnArray[currentTurn].GetComponent<BaseEntityClass>().getJob() + " used " + stringAttack + " protecting " + currentSelection.GetComponent<BaseEntityClass>().getJob() + "!";
            yield return new WaitForSeconds(1.5f);
        }       
        else if (stringAttack == "Meditate")
        {
            runText.text = turnArray[currentTurn].GetComponent<BaseEntityClass>().getJob() + " used " + stringAttack + " regaining " + manaRegen + " mana!";
            yield return new WaitForSeconds(1.5f);
        }
        else if (stringAttack == "Taunt")
        {
            runText.text = turnArray[currentTurn].GetComponent<BaseEntityClass>().getJob() + " used " + stringAttack + " on " + currentSelection.GetComponent<BaseEntityClass>().getJob() + "!";
            yield return new WaitForSeconds(1.5f);
        }

        //Single target attack
        else if (!isMulti && isAttack)
        {
            if (turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamageValue() > 0)
            {
                if (isPlayerAttacking == false)
                {
                    runText.text = turnArray[currentTurn].GetComponent<BaseEntityClass>().getJob() + " used " + stringAttack + " against " + target.getJob() + " dealing " + damageVal + " damage!";
                }
                else
                {
                    runText.text = turnArray[currentTurn].GetComponent<BaseEntityClass>().getJob() + " used " + stringAttack + " against " + currentSelection.GetComponent<BaseEntityClass>().getJob()
                        + " dealing " + damageVal + " damage!";
                }
            }
            yield return new WaitForSeconds(1.5f);
        }
        ...
        else if (isMulti && isAttack)
        {
            if (isPlayerAttacking == false)
            {
                runText.text = turnArray[currentTurn].GetComponent<BaseEntityClass>().getJob() + " used " + stringAttack + "! Dealing ";                
            ...
            {          
                ...
                if (multiSelect1.GetComponent<BaseEntityClass>().getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage1() > 0)
                {
                    runText.text += damageVal1 + " damage to " + multiSelect1.GetComponent<BaseEntityClass>().getJob() + "! ";
                }

                if (multiSelect2.GetComponent<BaseEntityClass>().getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage2() > 0)
                {
                    runText.text += damageVal2 + " damage to " + multiSelect2.GetComponent<BaseEntityClass>().getJob() + "! ";
                }

                if (multiSelect3.GetComponent<BaseEntityClass>().getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage3() > 0)
                {
                    runText.text += damageVal3 + " damage to " + multiSelect3.GetComponent<BaseEntityClass>().getJob() + "! ";
                }

                if (multiSelect4.GetComponent<BaseEntityClass>().getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getDamage4() > 0)
                {
                    runText.text += damageVal4 + " damage to " + multiSelect4.GetComponent<BaseEntityClass>().getJob() + "! ";
                }
            }
            yield return new WaitForSeconds(2.5f);
        }
        ...
        else if (!isMulti && !isAttack)
        {
            if(isPlayerAttacking == false)
            {
                runText.text = turnArray[currentTurn].GetComponent<BaseEntityClass>().getJob() + " used " + stringAttack + ". " + target.GetComponent<BaseEntityClass>().getJob()
                    + " regained " + healthRegen + " health!";
            }
            else
            {
                runText.text = turnArray[currentTurn].GetComponent<BaseEntityClass>().getJob() + " used " + stringAttack + ". " + currentSelection.GetComponent<BaseEntityClass>().getJob() 
                    + " regained " + healthRegen + " health!";
            }
            yield return new WaitForSeconds(1.5f);
        }
        ...
        else if (isMulti && !isAttack)
        {
            ...
            if (isPlayerAttacking == false)
            {
                runText.text = turnArray[currentTurn].GetComponent<BaseEntityClass>().getJob() + " used " + stringAttack + "! Healing ";

                if (target1.getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal1() > 0)
                {
                    runText.text += healVal1 + " health to " + target1.getJob() + "! ";
                }

                if (target2.getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal2() > 0)
                {
                    runText.text += healVal2 + " health to " + target2.getJob() + "! ";
                }

                if (target3.getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal3() > 0)
                {
                    runText.text += healVal3 + " health to " + target3.getJob() + "! ";
                }

                if (target4.getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal4() > 0)
                {
                    runText.text += healVal4 + " health to " + target4.getJob() + "! ";
                }
            }
            else
            {
                ...
                if (multiSelect1.GetComponent<BaseEntityClass>().getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal1() > 0)
                {
                    runText.text += healVal1 + " health to " + multiSelect1.GetComponent<BaseEntityClass>().getJob() + "! ";
                }

                if (multiSelect2.GetComponent<BaseEntityClass>().getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal2() > 0)
                {
                    runText.text += healVal2 + " health to " + multiSelect2.GetComponent<BaseEntityClass>().getJob() + "! ";
                }

                if (multiSelect3.GetComponent<BaseEntityClass>().getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal3() > 0)
                {
                    runText.text += healVal3 + " health to " + multiSelect3.GetComponent<BaseEntityClass>().getJob() + "! ";
                }

                if (multiSelect4.GetComponent<BaseEntityClass>().getIsAlive() != false && turnArray[currentTurn].GetComponent<BaseEntityClass>().getHeal4() > 0)
                {
                    runText.text += healVal4 + " health to " + multiSelect4.GetComponent<BaseEntityClass>().getJob() + "! ";
                }
            }
            yield return new WaitForSeconds(2.5f);
        }
